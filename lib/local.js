          ...
          if (theuser) {
            ...
            if(!theuser.local || !theuser.local.derived_key) {
              return done(null, false, {
                ...
                i18n: {
                  key: c.INVALID_USERNAME_OR_PASSWORD
                }
              });
            }
  ...
  function handleFailedLogin(userDoc, req, done) {
    ...
    return user.handleFailedLogin(userDoc, req)
      .then(function(locked) {
        if(locked) {
          var lockoutTime = Math.round(config.getItem('security.lockoutTime') / 60);
          invalid.message = 'Maximum failed login attempts exceeded. Your account has been locked for ' + lockoutTime + ' minutes.';

          invalid.i18n = {
            key: c.MAX_LOGIN_ATTEMPTS_EXCEEDED,
            params: {
