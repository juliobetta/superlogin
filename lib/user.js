...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.create = function (form, req, sendEmail) {
    ...
    return user.process()
      ...
      .then(function(result) {
        ...
        if(!sendEmail || !config.getItem('local.sendConfirmEmail')) {
          return BPromise.resolve();
        }
        return mailer.sendEmail(emailTemplate, newUser.unverifiedEmail.email, {req: req, user: newUser});
      })
      .then(function () {
      ...
      .then(function(theUser) {
        ...
        if(!provider) {
          return BPromise.reject({
            ...
            i18n: {
              key: c.MUST_SPECIFY_PROVIDER_TO_UNLINK
            },
            status: 400
          });
        }
        ...
        if(!user.providers || !(user.providers instanceof Array) || user.providers.length < 2) {
          return BPromise.reject({
            ...
            i18n: {
              key: c.CANNOT_UNLINK_ONLY_PROVIDER
            },
            status: 400
          });
        }
        ...
        if(user.local && user.local.salt && user.local.derived_key) {
          ...
          if(!form.currentPassword){
            return BPromise.reject({
              ...
              i18n: {
                key: c.MUST_SPECIFY_PASSWORD_TO_CHANGE
              },
              status: 400
            });
          }
  ...
  this.forgotPassword = function(email, req, emailTemplate) {
    ...
    return userDB.query('auth/email', {key: email, include_docs: true})
      ...
      .then(function() {
        ...
        return mailer.sendEmail(emailTemplate, user.email || user.unverifiedEmail.email,
    ...
    if(user_id) {
      ...
      if(!session_id) {
        return BPromise.reject({
          ...
          i18n: {
            key: c.USER_ID_OR_SESSION_ID_NOT_SPECIFIED
          },
          status: 401
        });
      }
